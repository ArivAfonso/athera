...
const DetailsPage = ({}) => {
        ...
        async function fetchData() {
            try {
                ...
                if (session.session == null) {
                    ...
                    setTopics(topics?.map((topic) => topic.name))
                }
            } catch (err) {
                console.log(err)
            }
    ...
    const checkIncludes = (topic: String) => {
        return interests?.some((item) => item === topic)
    }
    ...
    const setNewTopics = (topic: string) => {
        if (!checkIncludes(topic)) {
            setInterests((prevTopics) => [...prevTopics, topic])
        }
    }
    ...
    async function submitInterests() {
        ...
        //Create an array with id and interest
        const myInterests = interests.map((interest) => ({
            user_id: session.session?.user?.id,
            topic: interest,
        }))

        try {
            await supabase.from('interests').insert(myInterests)
            ...
            {part2 ? (
                <>
                    <header className="text-center max-w-2xl mx-auto - mb-14 sm:mb-16 lg:mb-20">
                        <Heading2>Your Interests</Heading2>
                        <span className="block text-sm mt-2 text-neutral-700 sm:text-base dark:text-neutral-200">
                            All your interests are private. Choose at least one.
                        </span>
                    </header>
                    <div className="flex flex-col items-center">
                        ...
                        <div className="flex flex-wrap items-center justify-center w-full md:w-[75%]">
                            {interests?.map((interest, index) => (
                                <>
                                    <button
                                        ...
                                        className="flex items-center justify-center mr-2 my-2 px-2 py-1.5 rounded bg-neutral-100 disabled:hover:bg-neutral-100 dark:bg-neutral-700 dark:hover:bg-neutral-6000 dark:disabled:hover:bg-neutral-700 hover:bg-neutral-200 disabled:cursor-not-allowed disabled:opacity-70"
                                    >
                                        {interest} ({index + 1})
                                    </button>
                                </>
                            ...
                            {topics?.length ? (
                                ...
                                <Empty
                                    ...
                                    subText="Press enter to create a brand new Topic."
