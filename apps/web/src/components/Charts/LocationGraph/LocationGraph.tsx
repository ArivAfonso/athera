...
interface SaleLocationDataItem {
    key: number
    region: string
    order: number
    revenue: number
...
const SaleByLocation = React.memo(() => {
    ...
    if (salesLocation !== null) {
        //@ts-ignore
        salesLocation[locationTab].map((item: any[], key: number) => {
    ...
    const renderThumb: React.FC<RenderThumbProps> = ({ style }) => {
    ...
    return (
        <Cards title="Audience Demographics" size="large">
