...
export const getTwitterEmbedUrl = (
    options: GetTwitterEmbedUrlOptions
): string => {
...
export const loadTwitterScript = () => {
    return new Promise((resolve, reject) => {
        ...
        if (existingScript) {
            // If the script is already in the document, resolve the promise
...
export const fetchOEmbedHtml = async (params: Record<string, any>) => {
    ...
    const res = await fetch(
        `${window.location.origin}/api/editor/twitter?url=${urlParams.toString()}`
    )
    ...
    const html = await res.text() // Extract HTML content
    return html // Return HTML content as string
}
