...
const buttonTypes: Icons.IconKeys[] = [
    'Inline',
    'Left',
    'InlineCenter',
    'Right',
...
export const mediaPlacement = (options: MediaPlacement): void => {
    const {
        ...
        tooltip,
        tippyModal,
        iframe: targetElement,
        wrapper,
        extraActions,
    } = options
    const nodePos = editor.view.posAtDOM(wrapper, 0)
    const { attrs: mediaNodeAttrs = {} } =
        editor.state.doc.nodeAt(nodePos) || {}
    ...
    const [buttonInline, btnSquareLeft, btnSquareCenter, btnSquareRight] =
        buttonTypes.map((type) => {
            const btn = createElement('button', '', Icons[type]())
            btn.setAttribute('type', 'button')
            btn.classList.add('hypermultimedia__modal__btn')
            return btn
        })
    ...
    btnPlacementSettings(mediaMargin, [
        ...
        button.addEventListener('click', () => {
            return applyStyleAndAttributes(
                wrapper,
                style,
                attributes,
                editor,
    ...
    const marginDivider = createElement(
        'div',
        'hypermultimedia__modal__divider'
    )

    const selectMargin = createMarginSelection(
        nodePos,
        wrapper,
        tooltip,
        editor,
