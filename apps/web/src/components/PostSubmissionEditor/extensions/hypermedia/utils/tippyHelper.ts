...
class Tooltip {
    ...
    private createTooltip() {
        ...
        if (this.tippyInstance.popper.firstChild) {
            ;(
                this.tippyInstance.popper.firstChild as HTMLElement
            ).addEventListener('blur', this.tippyBlurHandler)
        }
    }
    update(
        view: EditorView,
        ...
        if (this.tippyInstance) {
            this.tippyInstance.setProps({
                ...
                getReferenceClientRect: () => {
                    ...
                    return nodePos
                        ? nodePos
                        : { ...posToDOMRect(view, pos, pos), width: 0 }
                },
            })
        }
    }
