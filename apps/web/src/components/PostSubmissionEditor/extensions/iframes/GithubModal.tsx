...
const GistModal: FC<Props> = ({ show, onCloseModal, onSubmit }) => {
    ...
    const handleClickSubmitForm = (e: any) => {
        ...
        if (!gistUrlPattern.test(gistUrl)) {
            setError('Please enter a valid GitHub Gist URL.')
            return
        }
        const embedUrl = `${gistUrl}.pibb?background=transparent`
        onSubmit(embedUrl)
        onCloseModal()
    }
    ...
    const renderContent = () => {
        return (
            <form action="/#" onSubmit={handleClickSubmitForm}>
                <Label>Type the URL of the GitHub Gist you want to embed</Label>
                <Input
                    required
                    className="mt-1"
                    type={'text'}
                    onChange={(e) => {
                        setGistUrl(e.currentTarget.value)
                        setError('')
                    }}
                    ref={initialFocusRef}
                />
    ...
    return (
        <Modal
            ...
            modalTitle="GitHub Gist Iframe URL"
