...
export interface MonthProps extends MonthBaseProps {
    children?: ReactNode
    className?: string
...
const Month = forwardRef<HTMLTableElement, MonthProps>((props, ref) => {
    ...
    const weekdays = getWeekdaysNames(firstDayOfWeek, weekdayLabelFormat).map(
        (weekday) => (
            <th key={weekday} className="h-7">
                <span>{weekday}</span>
            </th>
        )
    )
    ...
    const rows = days.map((row, rowIndex) => {
        const cells = row.map((date, cellIndex) => {
            ...
            return (
                <td
                    key={cellIndex}
                    className={classNames('text-center w-9 h-9 p-0')}
                >
                    <Day
                        ref={(button) => {
                            if (daysRefs) {
        ...
        return (
            <tr key={rowIndex} className={classNames('font-semibold')}>
                {cells}
            </tr>
        )
    })
    ...
    return (
        <table
            ref={ref}
            className={classNames('border-collapse w-full', className)}
            cellSpacing="0"
            {...rest}
        >
            {!hideWeekdays && (
